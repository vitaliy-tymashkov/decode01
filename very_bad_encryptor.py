...
if __name__ == "__main__":
    operation = raw_input(
        "[E] or [e] for encrypt, [D] or [d] for decrypt with user input, [any other] for decrypt crack.me.output with 62628183\n")
    if operation == 'E' or operation == 'e':
        ...
        key = raw_input("Please enter a numeric decryption pin up to 8 digits long\n")
        if key.isdigit() and len(key) < 9:
            ...
            with open(filePath + '.decrypted.txt', 'w') as f:
                f.write(decrypted)
        ...
        filePath = 'crack.me.output'
        key = '62628183'
        print "Working with [" + filePath + "] and key = [" + key + "]."
        if key.isdigit() and len(key) < 9:
